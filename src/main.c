...
int main(int argc, char** argv) {    
    ...
    int use_bh = 1;
    if(!read) {
        printf("%i frames, %i particles simulate\n", frames, particles);
        ...
        */
        ...
        generateCloud(particle_list, particles, (Vector2){0,0}, (Vector2){3, 3});
    ...
    InitWindow(640, 480, "Cosmology Simulation");
    ...
    while(!WindowShouldClose()) {
        ...
        BeginTextureMode(screen_texture);
            ...
            for(int j = 0; j < particles; j++) {
                ...
                DrawCircle(screencords.x, screencords.y, 1 + (current.size/1000), WHITE);
            }

            Vector2 screen_start = cartToScreen((Vector2){-screen_width/2, 0}, camdata, screen_texture);
            Vector2 screen_end = cartToScreen((Vector2){screen_width/2, 0}, camdata, screen_texture);
        ...
        BeginDrawing();
            ...
            DrawTextureRec(screen_texture.texture, (Rectangle){0,0, screen_width, screen_height},(Vector2){0,0}, WHITE);
